...
public class contract_Loader {
    ...
    private static void openDB(String host, String dbname,
        ...
        try {
            stmt = con.prepareStatement(contractLoader);
        } catch (SQLException e) {
    ...
    private static void loadData(String contract_number,String contract_date,String client_enterprise,String supply_center)
            throws SQLException {
        if (con != null) {
            stmt.setString(1, contract_number);
            stmt.setString(2, contract_date);
            stmt.setString(3, client_enterprise);
            stmt.setString(4, supply_center);
            stmt.addBatch();
        }
    }
...
//        }
        try (BufferedReader infile
            ...
            Set contract_number_list = new HashSet<>();
            int      cnt = 0;
            int      count = 0;
            // Empty target table
            ...
            while ((line = infile.readLine()) != null) {
                parts = line.split(",");
                if (parts.length > 1) {
                    if(!contract_number_list.contains(parts[0])) {
                        contract_number_list.add(parts[0]);
                        contract_number = parts[0];
                        contract_date = parts[11];
                        client_enterprise = parts[1];
                        supply_center = parts[2];
                        loadData(contract_number,contract_date,client_enterprise,supply_center);
                        cnt++;
                    }
                    count++;
                    if (cnt % BATCH_SIZE == 0) {
